...
public class PlayerKeysOnline implements KeyListener, Runnable {
    ...
    public PlayerKeysOnline(Player player, Socket con, boolean localPlayer) {
        this.con = con;
    ...
    public void run() {
        while (true) {
            ...
            } else { // IMplement logic for supoustly player 2, the one that is moving
                     // Through the instructions from the server
                if (movingUp && canMoveUp()) {
                    player.setFacingPosition("up");
                    player.modifyPlayerY(-MOVE_SPEED);
                }
